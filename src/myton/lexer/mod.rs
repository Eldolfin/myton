...
mod tests {
    ...
    fn test_lexer_case(input: &str, expected: Vec<TokenKind>) {
        ...
        let mut message = format!(
            "Lexer failed to tokenize correctly \"{}\", diff: {:?}\n",
            input, diff
        );
    ...
    #[test]
    fn test_lexer() {
        test_lexer_case("1+2", vec![Number, Plus, Number, Newline, Eof]);

        test_lexer_case(
            "(1*2) + 3 - 4/1.2",
            vec![
                LeftParen, Number, Star, Number, RightParen, Plus, Number, Minus, Number, Slash,
                Number, Newline, Eof,
            ],
        );
        ...
        test_lexer_case(
            "def main() -> int<>:",
            vec![
                Def, Identifier, LeftParen, RightParen, Minus, Greater, Identifier, Less, Greater,
                Colon, Newline, Eof,
            ],
        );
        ...
        test_lexer_case(
            "a=1\nb=2",
            vec![
                Identifier, Equal, Number, Newline, Identifier, Equal, Number, Newline, Eof,
            ],
        );
        ...
        test_lexer_case(
            "if 1:\n    print 1\nelse:\n    print \"lol\"",
            vec![
                If, Number, Colon, Newline, Print, Number, Newline, Else, Colon, Newline, Print,
                Stringue, Newline, Eof,
            ],
        );
